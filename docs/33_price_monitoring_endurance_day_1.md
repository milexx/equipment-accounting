...
| `dell_r740` | 200 | `no_data`, offline finding: `page_not_found` | 0 | 0 | 0 | 0 | 0 | - | - | - |
...
- `dell_r740` получил `HTTP 200`, но offline-анализ raw HTML нашёл страницу `Такой страницы не существует`. Это не рыночный `no_data`, а неподходящий search URL категории.
- Пауза между позициями применена.
- При сетевых исключениях worker теперь пишет `parser_error`, а не роняет весь run.
- Для будущих запусков worker доработан: такая страница будет классифицироваться как `parser_error` с причиной `page_not_found`, а не как `no_data`.
