...
3. Следующий продуктовый шаг: сделать редактирование карточки с `row_version`, чтобы пользователь не перетирал параллельные изменения.
4. Отдельный UX-шаг: показывать в форме только поля выбранного типа оборудования, а не все fieldset сразу.
