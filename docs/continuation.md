...
3. Следующий продуктовый шаг: сделать рабочие очереди центра в реестре:
   - на проверке;
   - доработка;
   - диагностика;
   - списание;
   - оценка/продажа.
4. После очередей - сделать редактирование карточки с `row_version`, чтобы пользователь не перетирал параллельные изменения.
