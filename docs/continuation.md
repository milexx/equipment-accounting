...
3. Следующий продуктовый шаг: сделать редактирование карточки с `row_version`, чтобы филиал мог исправлять возврат на доработку, а центр не перетирал параллельные изменения.
4. После редактирования - связать `/region` с настоящей авторизацией: регион брать из текущего пользователя, selector убрать.
5. Отдельный UX-шаг: показывать в форме только поля выбранного типа оборудования, а не все fieldset сразу.
